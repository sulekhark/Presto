...
if 'narrowor' in sys.argv:
    ...
    wideConsequents = { t for t in allConsequents if len(tuple2ConsequentClauses[t]) >= 4 }
    ...
    while changed:
        ...
        for t in wideConsequents:
            ...
            if len(tuple2ConsequentClauses[t1]) >= 4: wideConsequents.add(t1)
            if len(tuple2ConsequentClauses[t2]) >= 4: wideConsequents.add(t2)
...
allNodes = list(allClauses | allTuples)
