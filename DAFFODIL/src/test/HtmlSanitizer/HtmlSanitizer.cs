...
namespace Vereyon.Web
{
    ...
    public class HtmlSanitizer : IHtmlSanitizer
    {
        ...
        private string GetAttributeValueInt(HtmlNode node, string s1, string s2)
        {
            return node.GetAttributeValue(s1, s2);
        }
        private string TryGetAttributeValue(HtmlNode node, string s1, string s2)
        {
            string retStr = String.Empty;
            try
            {
                retStr = GetAttributeValueInt(node, s1, s2);
            }
            catch (ArgumentNullException ane) { if (!HandleException(ane)) throw ane; }
            catch (InvalidProgramException ipe) { if (!HandleException(ipe)) throw ipe; }
            catch (ArgumentOutOfRangeException aore) { if (!HandleException(aore)) throw aore; }
            catch (ArgumentException ae) { if (!HandleException(ae)) throw ae; }
            catch (IndexOutOfRangeException iore) { if (!HandleException(iore)) throw iore; }
            catch (Exception e) { throw e; }
            return retStr;
        }

        private void SetAttributeValueInt(HtmlNode node, string s1, string s2)
        {
            node.SetAttributeValue(s1, s2);
        }

        private void TrySetAttributeValue(HtmlNode node, string s1, string s2)
        {
            try
            {
                SetAttributeValueInt(node, s1, s2);
            }
            catch (ArgumentNullException ane) { if (!HandleException(ane)) throw ane; }
            catch (InvalidProgramException ipe) { if (!HandleException(ipe)) throw ipe; }
            catch (ArgumentOutOfRangeException aore) { if (!HandleException(aore)) throw aore; }
        ...
        private string WriteToInt(HtmlNode node)
        {
            return node.WriteTo();
        }

        private string TryWriteTo(HtmlNode node)
        {
            string retStr = String.Empty;
            try
            {
                retStr = WriteToInt(node);
            }
            catch (ArgumentNullException ane) { if (!HandleException(ane)) throw ane; }
            catch (InvalidProgramException ipe) { if (!HandleException(ipe)) throw ipe; }
            catch (ArgumentOutOfRangeException aore) { if (!HandleException(aore)) throw aore; }
        ...
        private void LoadHtmlInt(HtmlDocument hDoc, string html)
        {
            hDoc.LoadHtml(html);
        }

        private void TryLoadHtml(HtmlDocument hDoc, string html)
        {
            try
            {
                LoadHtmlInt(hDoc, html);
            }
            catch (ArgumentNullException ane) { if (!HandleException(ane)) throw ane; }
            catch (InvalidProgramException ipe) { if (!HandleException(ipe)) throw ipe; }
            catch (ArgumentOutOfRangeException aore) { if (!HandleException(aore)) throw aore; }
        ...
        private HtmlNode CreateNodeInt(string s)
        {
            return HtmlTextNode.CreateNode(s);
        }

        private HtmlNode TryCreateNode(string s)
        {
            HtmlNode node = null;
            try
            {
                node = CreateNodeInt(s);
            }
            catch (ArgumentNullException ane) { if (!HandleException(ane)) throw ane; }
            catch (InvalidProgramException ipe) { if (!HandleException(ipe)) throw ipe; }
            catch (ArgumentOutOfRangeException aore) { if (!HandleException(aore)) throw aore; }
        ...
        private HtmlNode ReplaceChildInt(HtmlNode parent, HtmlNode replNode, HtmlNode node)
        {
            return parent.ReplaceChild(replNode, node);
        }

        private HtmlNode TryReplaceChild(HtmlNode parent, HtmlNode replNode, HtmlNode node)
        {
            HtmlNode retNode = null;
            try
            {
                retNode = ReplaceChildInt(parent, replNode, node);
            }
            catch (ArgumentNullException ane) { if (!HandleException(ane)) throw ane; }
            catch (InvalidProgramException ipe) { if (!HandleException(ipe)) throw ipe; }
            catch (ArgumentOutOfRangeException aore) { if (!HandleException(aore)) throw aore; }
        ...
        private void InsertBeforeInt(HtmlNode parent, HtmlNode refNode, HtmlNode node)
        {
            parent.InsertBefore(refNode, node);
        }

        private void TryInsertBefore(HtmlNode parent, HtmlNode refNode, HtmlNode node)
        {
            try
            {
                InsertBeforeInt(parent, refNode, node);
            }
            catch (ArgumentNullException ane) { if (!HandleException(ane)) throw ane; }
            catch (InvalidProgramException ipe) { if (!HandleException(ipe)) throw ipe; }
            catch (ArgumentOutOfRangeException aore) { if (!HandleException(aore)) throw aore; }
		...
		public void SanitizeNode(HtmlNode node)
        {
            ...
            using (new RecursionGuard(this))
            {
                ...
                {
                    try { node.Remove();}
                    catch (Exception e) { if (!HandleException(e)) throw e; }
                    return;
                }
                ...
                if (node.NodeType == HtmlNodeType.Text && EncodeHtmlEntities)
                {
                    ...
                    var replacement = TryCreateNode(entitized);
                    TryReplaceChild(node.ParentNode, replacement, node);
                    return;
                }
                ...
                if (rule != null)
                {
                    ...
                    {
                        var className = TryGetAttributeValue(node, "class", string.Empty);
                        if (string.IsNullOrEmpty(className))
                            className = rule.SetClass;
                        else
                            className += " " + rule.SetClass;
                        TrySetAttributeValue(node, "class", className);
                    }
            ...
            switch (operation)
            {
                case SanitizerOperation.FlattenTag:
                    ...
                    foreach (var child in node.ChildNodes)
                        TryInsertBefore(node.ParentNode, child, node);
                    try { node.Remove(); }
                    catch (Exception e) { if (!HandleException(e)) throw e; }
                    return false;
        ...
        /// <summary>
        /// Uniformly processes an exception. 
        /// </summary>
        /// <returns>
        /// Returns true if handled and false if the exception needs to be thrown.
        /// </returns>
